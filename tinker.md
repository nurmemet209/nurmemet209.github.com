...
其实tinker官方接入官方文档已经很详细了，我这里只列出了一些初学者难理解的问题  
[tinker官方首页](https://github.com/Tencent/tinker)  

首先是根目录build.gralde文件添加  
```groovy
//注意版本，查看官方文档使用最新版本
 classpath ('com.tencent.tinker:tinker-patch-gradle-plugin:1.7.3')
```

在项目build.gradle文档中dependencies添加
```groovy
...
在项目build.gradle文档中defaultConfig中添加
```groovy
android {
...
ext {
    ...
    tinkerOldApkPath = "${bakPath}/oldversion2.apk"
    //proguard mapping file to build patch apk
    //tinkerApplyMappingPath = "${bakPath}/app-debug-1018-17-32-47-mapping.txt"
    tinkerApplyMappingPath = "${bakPath}/"
    //resource R.txt to build patch apk, must input if there is resource changed
    tinkerApplyResourcePath = "${bakPath}/pppcar-debug-1117-20-58-02-R.txt"
...
然后修改MyApplication.java文件，是因为Tinker的DefaultApplicationLike不是继承自android的Application类所以所以tinker插件在编译的
时候会动态生成Application类以下注解中的com.cn.pppcar.CustomApplication类是Tinker插件编译的时候要生成的Application的名称，
这个要注册到mainfest.xml文件中,application标签的name属性
```java
...
@DefaultLifeCycle(application = "com.cn.pppcar.CustomApplication",
...
public class MyApplication extends DefaultApplicationLike {
    ...
    //提供个getApplicationContext方法供application的OnCreate中方法的其他其他初始化操作中使用
...
每次编译的时候项目build->back目录下会生成apk文件资源id映射文件  
![](images/tinker_1.png)  
然后修改好代码后配置下以下文件,老版本和编译老版本的时候的资源映射文件  
```groovy
  //old apk file to build patch apk
    tinkerOldApkPath = "${bakPath}/oldversion2.apk"
    //resource R.txt to build patch apk, must input if there is resource changed
...
然后控制带执行gradle pppcar:tinkerPatchDebug ,pppcar是我的项目名称,编译成功之后会提示差分包生成路径，是根目录下会生成
tinkerPatch文件，是差分包生成目录，差分包放在手机sd卡中,MainActivity中调用此方法，如果安装成功会提示patch success please restart process
```java
